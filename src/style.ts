...
import type { Size, StyledProps } from "./types";
...
export const StyledFramerCounter = styled(m.div, options)<Props>`
	align-items: center;
	...
	padding: ${(props) => 7 * sizeToScale(props.size)}px ${(props) => 17 * sizeToScale(props.size)}px;
	...
	${(props) =>
		...
		css`
			&:hover > div > button:nth-of-type(2) {
				box-shadow: 0 ${20 * sizeToScale(props.size)}px ${25 * sizeToScale(props.size)}px ${28 * sizeToScale(props.size)}px rgb(0 0 0 / 0.1),
					0 ${1.5 * sizeToScale(props.size)}px ${2 * sizeToScale(props.size)}px ${-1 * sizeToScale(props.size)}px rgb(0 0 0 / 0.5);
			}
		`}
	...
	& > div {
		...
		& > button:nth-of-type(2n + 1) {
			transition: border-width 50ms, color ${(props) => (props.isDragging ? 0 : 150)}ms, background-color 150ms;
		...
		& > button:nth-of-type(2) {
			...
			box-shadow: 0 ${(props) => 5 * sizeToScale(props.size)}px ${(props) => 5 * sizeToScale(props.size)}px ${(props) => -2 * sizeToScale(props.size)}px rgb(0 0 0 / 0.1),
				0 ${(props) => 1.5 * sizeToScale(props.size)}px ${(props) => 2 * sizeToScale(props.size)}px ${(props) => -1 * sizeToScale(props.size)}px rgb(0 0 0 / 0.5);
